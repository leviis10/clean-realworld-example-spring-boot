...
import com.leviis.realworldexample.article.adapter.inbound.http.dto.request.UpdateArticleRequest;
...
public final class ArticleController {
    ...
    @GetMapping("/{slug}")
    public ResponseEntity<ResponseWrapper<ArticleResponse>> getArticle(
    ...
    public ResponseEntity<ResponseWrapper<ArticleResponse>> updateBySlug(
        ...
        final ArticleWithBodyAndAuthor updatedArticle = updateArticleUseCase.execute(UpdateArticleCommand.builder()
                .setAuthenticatedUser(userContext.intoUserDomain())
                .setSlug(Slug.from(SlugUtils.getTitleFrom(slug), SlugUtils.getIdFrom(slug)))
                .setUpdateDataDto(request.into(UpdateArticleCommand.UpdateDataDto.class))
                .build());
        return ResponseEntity.status(HttpStatus.OK)
